...
class AnalysisDriver {
  ...
  Future<Uint8List> buildPackageBundle({
    ...
    // OPTIMIZATION: Parallel library loading with batch processing
    const batchSize = 10; // Process in batches to avoid excessive memory usage
    for (var i = 0; i < uriList.length; i += batchSize) {
      var batch = uriList.skip(i).take(batchSize).toList();

      // Load libraries in parallel within each batch
      ...
      // Write results sequentially (bundleWriter is not thread-safe)
  ...
  Future<SomeLibraryElementResult> getLibraryByUri(String uri) async {
    ...
    if (_pendingFileChanges.isEmpty) {
      final _cacheCheckStart = DateTime.now();
