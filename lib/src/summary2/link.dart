...
class Linker {
  ...
  void _buildExportScopes() {
    ...
    // OPTIMIZATION: Use topological sort instead of iterative convergence
    // Build dependency graph: library -> libraries it exports to
    var outgoingEdges = <LibraryBuilder, Set<LibraryBuilder>>{};
    var incomingCount = <LibraryBuilder, int>{};

    for (var library in both) {
      outgoingEdges[library] = {};
      incomingCount[library] = 0;
    }
    ...
    for (var exported in both) {
      for (var export in exported.exports) {
        ...
        if (both.contains(exporter)) {
          outgoingEdges[exported]!.add(exporter);
          incomingCount[exporter] = (incomingCount[exporter] ?? 0) + 1;
        }
      }
    }
    ...
    while (queue.isNotEmpty) {
      ...
      processed.add(exported);

      // Propagate exports to dependent libraries
      for (var export in exported.exports) {
      ...
      // Update incoming counts and add newly ready nodes
      for (var exporter in outgoingEdges[exported]!) {
        incomingCount[exporter] = incomingCount[exporter]! - 1;
        if (incomingCount[exporter] == 0 && !processed.contains(exporter)) {
          queue.add(exporter);
        }
      }
    }

    // Handle any remaining libraries (circular dependencies)
    // Fall back to iterative approach for circular parts
    var remaining = both.difference(processed);
    if (remaining.isNotEmpty) {
      var maxIterations = remaining.length; // Prevent infinite loops
      for (var iteration = 0; iteration < maxIterations; iteration++) {
        var hasChanges = false;
        for (var exported in remaining) {
          for (var export in exported.exports) {
            exported.exportScope.forEach((name, reference) {
